...
sealed class SuspensionManager {
	const string SESSION_STATE_FILENAME = "_sessionState.xml";
	...
	/// </summary>
	public static Dictionary<string, object> SessionState { get; private set; } = new();
		...
		try {
			...
			StorageFile file =
				await ApplicationData.Current.LocalFolder.CreateFileAsync(
					SESSION_STATE_FILENAME, CreationCollisionOption.ReplaceExisting);
	...
	public static async Task RestoreAsync() {
		...
		try {
			...
			StorageFile file =
				await ApplicationData.Current.LocalFolder.GetFileAsync(SESSION_STATE_FILENAME);
			using (IInputStream inStream = await file.OpenSequentialReadAsync()) {
				// Deserialize the Session State
				DataContractSerializer serializer = new(typeof(Dictionary<string, object>), KnownTypes);
				SessionState =
					(Dictionary<string, object>)serializer.ReadObject(inStream.AsStreamForRead());
			}
	...
	static readonly List<WeakReference<Frame>> RegisteredFrames = [];
	...
	public static void RegisterFrame(Frame frame, string sessionStateKey) {
		if (frame.GetValue(FrameSessionStateKeyProperty) != null) {
			throw new InvalidOperationException(
				"Frames can only be registered to one session state key");
		}
	...
	public static void UnregisterFrame(Frame frame) {
		...
		RegisteredFrames.RemoveAll(reference => !reference.TryGetTarget(out var testFrame) || testFrame == frame);
	}
